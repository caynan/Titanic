This is to explain where everything is. If you look into requirement.txt, it will show you the libraries that need to be
installed to run the code (open using Word, it is all run together in notepad). The easiest way to install the libraries
is to use pip. This tool can be downloaded quite easily. Once downloaded, it's as easy as typing 'pip install <library>' in
the command prompt from an Apple or Linux machine. Windows has other ways to download the libraries.
The data folder holds the training data (the test.csv file is not used for our programs). 
